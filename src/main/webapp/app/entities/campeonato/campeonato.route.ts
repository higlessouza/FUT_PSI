import { Injectable } from '@angular/core';
import { HttpResponse } from '@angular/common/http';
import { Resolve, ActivatedRouteSnapshot, Routes, Router } from '@angular/router';
import { Observable, of, EMPTY } from 'rxjs';
import { flatMap } from 'rxjs/operators';

import { Authority } from 'app/shared/constants/authority.constants';
import { UserRouteAccessService } from 'app/core/auth/user-route-access-service';
import { ICampeonato, Campeonato } from 'app/shared/model/campeonato.model';
import { CampeonatoService } from './campeonato.service';
import { CampeonatoComponent } from './campeonato.component';
import { CampeonatoDetailComponent } from './campeonato-detail.component';
import { CampeonatoUpdateComponent } from './campeonato-update.component';

@Injectable({ providedIn: 'root' })
export class CampeonatoResolve implements Resolve<ICampeonato> {
  constructor(private service: CampeonatoService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<ICampeonato> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((campeonato: HttpResponse<Campeonato>) => {
          if (campeonato.body) {
            return of(campeonato.body);
          } else {
            this.router.navigate(['404']);
            return EMPTY;
          }
        })
      );
    }
    return of(new Campeonato());
  }
}

export const campeonatoRoute: Routes = [
  {
    path: '',
    component: CampeonatoComponent,
    data: {
      authorities: [Authority.USER],
      pageTitle: 'futPsiApp.campeonato.home.title'
    },
    canActivate: [UserRouteAccessService]
  },
  {
    path: ':id/view',
    component: CampeonatoDetailComponent,
    resolve: {
      campeonato: CampeonatoResolve
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'futPsiApp.campeonato.home.title'
    },
    canActivate: [UserRouteAccessService]
  },
  {
    path: 'new',
    component: CampeonatoUpdateComponent,
    resolve: {
      campeonato: CampeonatoResolve
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'futPsiApp.campeonato.home.title'
    },
    canActivate: [UserRouteAccessService]
  },
  {
    path: ':id/edit',
    component: CampeonatoUpdateComponent,
    resolve: {
      campeonato: CampeonatoResolve
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'futPsiApp.campeonato.home.title'
    },
    canActivate: [UserRouteAccessService]
  }
];
